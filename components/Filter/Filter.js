...
const Filter = ({testFunction, brands, brandClickHandler, lookUpValueHandler, loading, resetValues}) => {
  ...
  return (
    <>
      ...
      <div className={styles.filter} style={{display: show ? 'flex' : 'none'}}>
		  <div className={styles.categories}>
		  <div className={styles.close} onClick={() => closeFilter()}>{closeIcon}</div>
            <Form
            	form={form} 
				      name="name"
				      layout="vertical"
					...
					buscar
						...
						<Collapse onChange={callback} ghost expandIconPosition='right'>
							<Panel
								...
								key="1"
							>
								<Link href="/products/equipamiento" passHref  ><li onClick={()=>twoFunctions()}>Todos</li></Link> 
								<Link href="/products/equipamientoVenta" passHref  ><li onClick={()=>twoFunctions()}>Venta</li></Link> 
								<Link href="/products/equipamientoAlquiler" passHref  ><li onClick={()=>twoFunctions()}>Alquileres</li></Link>
								<Link href="/products/equipamientoSt" passHref  ><li onClick={()=>twoFunctions()}>Servicio técnico</li></Link> 
							</Panel>
							<li className={styles.category}>
								<img src="/maskIcon.png" />
								<div>
									<Link href="/products/insumosMedicos" passHref><a onClick={()=>twoFunctions()}>Insumos médicos</a></Link>
								</div>
							</li>
							<li className={styles.category}>
								<img src="/bandageIcon.png" />
								<div>
									<Link href="/products/medicinaDeportiva" passHref><a onClick={()=>twoFunctions()}>Medicina deportiva</a></Link>
								</div>
							</li>
						</Collapse>
